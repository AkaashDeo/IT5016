...
- Movie Class: Manages movie attributes and status.
Author: Akaash Deo"""
...
class Movie:
    def __init__(self, title, genre, year):
        ...
        self.available = True  # Availability status of the movie, initially available
    ...
    def __str__(self):
        ...
        availability = "Available" if self.available else "Rented"  # Determine availability status
...
class Customer:
    ...
    def rent_movie(self, movie):
        ...
        if movie.available:
            ...
            self.rented_movies.append(movie)  # Add the movie to the list of rented movies
            print(f"Rented: {movie.title}")  # Notify that the movie was rented
        else:
            print(f"{movie.title} is not available.")  # Notify if the movie is not available

    def return_movie(self, movie):
        # Return a rented movie
        if movie in self.rented_movies:  # Check if the movie is in the list of rented movies
            movie.mark_as_available()  # Mark the movie as available
            self.rented_movies.remove(movie)  # Remove the movie from the list of rented movies
            print(f"Returned: {movie.title}")  # Notify that the movie was returned
        else:
    ...
    def list_rented_movies(self):
        ...
        if self.rented_movies:
            print(f"{self.name}'s Rented Movies:")  # Header for the list of rented movies
...
class RentalStore:
    def __init__(self):
        ...
        self.movies = []  # List to store available movies in the rental store

    def add_movie(self, movie):
        # Add a movie to the store's inventory
        self.movies.append(movie)  # Add the movie to the list
        print(f"Movie added: {movie.title}")  # Notify that the movie was added

    def list_movies(self):
        # List all available movies in the store
        if self.movies:
            print("Available Movies in Store:")  # Header for the list of available movies
...
def display_menu():
    # Display the menu options for the movie rental system
    print("\nMovie Rental System")
...
# 3. Testing:
#    - Test each functionality to ensure it works as expected. This includes
#      adding movies, renting and returning movies, listing available and rented
#      movies, and handling invalid inputs.
